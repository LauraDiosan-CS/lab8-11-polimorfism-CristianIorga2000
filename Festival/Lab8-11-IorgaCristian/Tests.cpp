...
void Test::testShow() {
	...
	Show s1("10/10/6969", "Himalaya", 1000, 276);
	assert(s1.getDate() == "10/10/6969");
	assert(s1.getPlace() == "Himalaya");
	assert(s1.getAvailablePlaces() == 1000);
	assert(s1.getOccupiedPlaces() == 276);
	Show s2(s1);
	assert(s2.getDate() == "10/10/6969");
	assert(s2.getPlace() == "Himalaya");
	...
	assert(s2.getAvailablePlaces() == 1000);
	assert(s2.getOccupiedPlaces() == 276);
	s2.setDate("11/06/6921");
	assert(s2.getDate() == "11/06/6921");
	s2.setPlace("NewYork");
	assert(s2.getPlace() == "NewYork");
	s2.setAvaiablePlaces(2500);
	assert(s2.getAvailablePlaces() == 2500);
	s2.setOccupiedPlaces(0650);
	assert(s2.getOccupiedPlaces() == 0650);
	IE* s3 = s2.clone();
	assert(s2.equals(s3));
	delete s3;
	s3 = nullptr;
...
void Test::testFilm() {
	...
	assert(f5->getShows()[0] == Show("10/10/6906", "Viena", 450, 300));
	delete f5;
	f5 = nullptr;
	delete f4;
	f4 = nullptr;
...
void Test::testArtist() {
	...
	Artist a1(name, "06/06/6906", "Ferentari", 1000, 999);
	assert(*(a1.getName()) == *name);
	assert(a1.getShows().size() == 1);
	assert(a1.getShows()[0] == Show("06/06/6906", "Ferentari", 1000, 999));
	Show s("09/06/6919", "Himalaya", 100, 59);
	shows.emplace_back(s);
	...
	assert(a2.getShows().size() == 1);
	assert(a2.getShows()[0] == Show("06/06/6906", "Ferentari", 1000, 999));
	a2.setName(name1);
	assert(*(a2.getName()) == *name1);
	a2.setName(nullptr);
...
void Test::testRepo() {
	...
	strcpy_s(name, strlen("Bol") + 1, "Bol");
	IE* f = new Film(name, std::vector<std::string>(), "10/10/6906", "Himalaya", 100, 10);
	r.addElem(f);
	assert(r.getSize() == 3);
	assert(r.getElemPos(f) == 2);
	r.deleteElem(ar);
	assert(r.getSize() == 2);
	IE* a = new Artist(name, "11/11/6911", "NewYork", 500, 500);
	r.modifyElem(fi, a);
